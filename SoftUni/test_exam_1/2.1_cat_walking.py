...
# Като знаете колко калории приема котката на ден, колко пъти и по колко минути Деси разхожда котката си, напишете
# програма, която изчислява дали разходката е достатъчна. За всяка минута от разходката, котката гори по 5 калории.
...
walks: int = int(input())
number_walks: int = int(input())
calories: int = int(input())

walking_time: int = walks * number_walks
calories_burned: float = walking_time * 5
half_calories = calories / 2

if calories_burned >= half_calories:
